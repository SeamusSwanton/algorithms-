...
public class IterativeSearcherTest extends TestCase {
	...
	//as a binary search should perform in O-n-logn time, there will be an upper limit on how many reads an algorithm can perform to be considered correct
	...
	boolean logarithmicPerformance = false;

	public void testBinarySearchNormal() {
		//'normal' searches
		IntegerArray.reset();
		assertEquals(47, searcher.doBinarySearch(arrayToSearch, 42));
		logarithmicPerformance = arrayToSearch.getReads() <= LOG_BASE_2_OF_FILE_SIZE;		
		assertEquals(true, logarithmicPerformance);			
	}
	...
	public void testBinarySearchAtBounds() {		
		...
		logarithmicPerformance = arrayToSearch.getReads() <= LOG_BASE_2_OF_FILE_SIZE + 2;		
		assertEquals(true, logarithmicPerformance);			
	...
	public void testBinarySearchForDuplicatedItem() {
		...
		logarithmicPerformance = arrayToSearch.getReads() <= LOG_BASE_2_OF_FILE_SIZE + 4;		
		assertEquals(true, logarithmicPerformance);			
		
		//ten occurences of an element... should return the 1st occurrence
		IntegerArray.reset();
		assertEquals(980, searcher.doBinarySearch(arrayToSearch, 982));
		logarithmicPerformance = arrayToSearch.getReads() <= LOG_BASE_2_OF_FILE_SIZE + 10;		
		assertEquals(true, logarithmicPerformance);	
	}
